...
def get_authorization_code(signin_url):
    ...
    user_config_path = os.path.join(os.path.split(__file__)[0],
                                    'config/test-config-sample.yaml')
    ...
    browser = webdriver.Chrome('/home/temescal-mike/chromedriver')
    browser.get(signin_url)
    time.sleep(3)

    #form_userid = browser.find_element_by_id('userid')
    actions = ActionChains(browser)
    actions.send_keys('testuser_surdouskicards')
    actions.send_keys(Keys.TAB)
    actions.send_keys('U32GExu4SJ2qjyt$$')
    ...
    decoded_code = urllib.parse.unquote(code).encode('utf-8').decode('utf8')
    print(decoded_code)
    return decoded_code
